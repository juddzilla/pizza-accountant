...
// taken from https://github.com/leequixxx/currencies.json/blob/master/currencies.json
...
export default function Currencies(value, update) {
  function onChange({ nativeEvent, target }) {
    ...
    const symbol = text.split('-')[1].trim();
    update({ code, symbol });
  }

  function onSelect(currency) {
        update({ code: currency.code, name: currency.name, symbol: currency.symbolNative });
  }
